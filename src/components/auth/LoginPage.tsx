...
export const LoginPage: React.FC<LoginPageProps> = ({ onNavigate, onSuccess }) => {
  ...
  return (
    <div className="min-h-screen w-full flex bg-slate-50">
      ...
      <div className="flex-1 flex flex-col justify-center px-4 sm:px-8 md:px-12 lg:px-16 py-12 max-w-xl mx-auto w-full">
        ...
        <div className="mb-8">
          <button
            ...
            className="inline-flex items-center gap-2 mb-6 group text-left"
          >
          ...
          <p className="mt-2 text-sm text-slate-600 leading-relaxed">
            Sign in to continue building resumes that get noticed.
          </p>
        </div>

        {/* Error Alert */}
        {errorMessage && (
          <div className="mb-6 p-4 rounded-xl bg-rose-50 border border-rose-200 text-rose-800 text-xs flex items-start gap-2.5 animate-shake">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-3 bg-white hover:bg-slate-50 text-slate-800 font-semibold text-sm py-3 px-4 rounded-xl border border-slate-300 shadow-xs hover:border-slate-400 transition-all disabled:opacity-60 cursor-pointer"
        >
      ...
      <AuthVisualSide quote="Sign in to continue building resumes that get noticed." />
    </div>
  );
};
